...
def run_test(path, files, name):
    ...
    err_out = None
    try:
        out, err_out = proc.communicate(str.encode(
            open(path+"prog").read()), timeout=timeoutT)
        ...
        if "out" in files and interpreter_path is not None and (ret_codes is None or max(ret_codes) >= 3):
            ...
            temp_out.close()
            proc = Popen([interpreter_path, "temp_out"],
...
print(
    f"Tests finished with {Fore.RED}{errors} errors{Fore.WHITE} and {Fore.GREEN}{successes} successes{Fore.WHITE}")
if errors == 0:
